...
const main = async () => {
  ...
  await Object.entries(config["active-services"]).reduce(
    async (previousPromise, [service, params]) => {
      ...
      try {
        await catalog[service].remove({ ...params, ...config.global });
  ...
  for (const [service, success] of results) {
    if (success) {
      console.log(`\x1b[32m✅ ${service} has been safely removed\x1b[0m`);
    } else {
      console.log(`\x1b[31m❌ Couldn't remove ${service}\x1b[0m`);
    }
  }
